...
public class Oauth extends Weibo {
	...
	public void setRedirectURI(String redirectURI) {
		if (redirectURI == null) {
			this.redirectURI = WeiboConfig.getValue("redirect_URI").trim();
		} else {
			this.redirectURI = redirectURI.trim();
		}
		
		log.debug("redirectURI is"+this.redirectURI);
	}
	...
	public AccessToken getAccessTokenByCode(String code) throws WeiboException {
		return new AccessToken(client.post(WeiboConfig.getValue("accessTokenURL"), new PostParameter[] { new PostParameter("client_id", WeiboConfig.getValue("client_ID")),
				...
				new PostParameter("redirect_uri", getRedirectURI()) }, false, null));
	}

	public String authorize(String response_type) throws WeiboException {
		return WeiboConfig.getValue("authorizeURL").trim() + "?client_id=" + WeiboConfig.getValue("client_ID").trim() + "&redirect_uri=" + getRedirectURI().trim()
				+ "&response_type=" + response_type;
	}

	public String authorize(String response_type, String state) throws WeiboException {
		
		String url =  WeiboConfig.getValue("authorizeURL").trim();
		url +=  "?client_id=";
		url +=WeiboConfig.getValue("client_ID").trim();
		url+="&redirect_uri=";
		url+=getRedirectURI().trim();
		url+="&response_type=";
		url+=response_type;
		url+="&state=";
		url+=state;

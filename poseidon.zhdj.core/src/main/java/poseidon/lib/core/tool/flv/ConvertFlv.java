package poseidon.lib.core.tool.flv;
...
public class ConvertFlv {
	...
	public static void main(String[] args) {
		ConvertFlv.convert("E:/snake.mp4", "E:/snake1.flv");
	}
	...
	private static boolean processFLV(String inputFile, String outputFile) {
		...
		// 低精度
		commend.add(getValue("flvPath") + "ffmpeg");
		commend.add("-i");
	...
	private static String processAVI(int type, String inputFile) {
		File file = new File(getValue("flvTempPath") + "temp.avi");
		if (file.exists())
			file.delete();
		List<String> commend = new java.util.ArrayList<String>();
		commend.add(getValue("flvPath") + "mencoder");
		commend.add(inputFile);
		commend.add("-oac");
		commend.add("mp3lame");
		...
		commend.add("avi");
		commend.add("-o");
		commend.add(getValue("flvTempPath") + "temp.avi");
		StringBuffer test = new StringBuffer();
		for (int i = 0; i < commend.size(); i++)
			test.append(commend.get(i) + " ");
		System.out.println(test);
